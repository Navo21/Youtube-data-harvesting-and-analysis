...
with col1:
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyDeWV_K2hx4wfv4lYQHtUHD5IZfSriwLLo'
...
# with col2:
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        mycursor.execute ("CREATE DATABASE IF NOT EXISTS youtube_db3")
        ...
        # Convert the ISO 8601 datetime strings to datetime objects
        video_df['Published_date'] = pd.to_datetime (video_df['Published_date'])

        # Create an SQLAlchemy engine
        engine = create_engine ('mysql+mysqlconnector://root:123456789!1@localhost:3306/youtube_db3')

        # Specify the dtype dictionary for the data types
        dtype = {
            ...
            'Published_date': sqlalchemy.DateTime,  # Change the data type to DateTime
        ...
        # Insert the DataFrame into the 'video' table
        video_df.to_sql ('video', engine, if_exists='append', index=False, dtype=dtype)

        # Convert the ISO 8601 datetime strings to datetime objects
        Comments_df['Comment_Published_date'] = pd.to_datetime (Comments_df['Comment_Published_date'], errors='coerce')

        # Filter out rows with 'Unavailable' values in certain columns
        valid_comments_df = Comments_df[
            (Comments_df['Video_Id'] != 'Unavailable') &
            (Comments_df['Comment_Id'] != 'Unavailable') &
            (Comments_df['Comment_Text'] != 'Unavailable') &
            (Comments_df['Comment_Author'] != 'Unavailable') &
            (pd.notnull (Comments_df['Comment_Published_date']))
            ]
        ...
        dtype = {
            ...
            'Comment_Published_date': sqlalchemy.DateTime
        }

        # Insert the valid DataFrame into the 'comments' table
        valid_comments_df.to_sql ('comments', engine, if_exists='append', index=False, dtype=dtype)
...
connect_for_question = pymysql.connect (host='localhost', user='root', password='123456789!1', db='youtube_db3')
...
# ===============================================   /   COMPLETED   /   ====================================================================== ## [Youtube API libraries]
import googleapiclient.discovery
from googleapiclient.discovery import build
